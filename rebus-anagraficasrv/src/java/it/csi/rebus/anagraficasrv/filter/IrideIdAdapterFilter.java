...
*/
package it.csi.rebus.anagraficasrv.filter;
...
import it.csi.rebus.anagraficasrv.common.UserMock;
...
import it.csi.rebus.anagraficasrv.integration.iride.MalformedIdTokenException;
...
public class IrideIdAdapterFilter implements Filter {
	...
	protected static final Logger LOG = Logger
			.getLogger(it.csi.rebus.anagraficasrv.util.Constants.COMPONENT_NAME + ".security");
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain fchn)
		...
		if (hreq.getSession().getAttribute(IRIDE_ID_SESSIONATTR) == null) {
			...
			if (devmode) {
				try {
					identita = irideServFacade.identificaUserPassword(UserMock.DEMO24, "********");
			...
			if (marker != null) {
				try {
					...
					try {
						...
						if (!authority.contains(AuthorizationRoles.ACCESSO_AL_SISTEMA_ANAGRAFICHE.getId())) {
							throw new UtenteNonAbilitatoException(Messages.UTENTE_NON_AUTORIZATO);
						}
						userInfo.setAuthority(authority);
						userInfo = userService.getDettaglioFunzionario(userInfo);
					} catch (Exception e) {
						if (e instanceof UtenteNonAbilitatoException) {
							throw (UtenteNonAbilitatoException) e;
						}
						LOG.error("[IrideIdAdapterFilter::doFilter] " + e.toString(), e);
					}
					...
					if (e instanceof UtenteNonAbilitatoException) {
						...
						response.getWriter().append("{ \"message\":\"").append(e.getMessage()).append("\" }");
						return;
					}
