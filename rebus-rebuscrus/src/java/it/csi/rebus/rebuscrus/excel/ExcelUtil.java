...
public class ExcelUtil {
	...
	private static final Logger LOGGER = Logger
			.getLogger(it.csi.rebus.rebuscrus.util.Constants.COMPONENT_NAME + ".ExcelUtil");
	
	public static enum EXCEL_PATTERN {
		STRING_PATTERN, NUMBER_3_DIGIT_PATTERN, EURO_PATTERN, INT_PATTERN, DATE_PATTERN, ANNO_PATTERN,EXTEND_DATE_PATTERN
	}
	...
	public static WritableCellFormat getRightAlignedStyle(WritableFont font,
			String dataPattern) {
		...
		try {
			...
			LOGGER.error("EcxelUtilGamOpera::WritableCellFormat: "
					+ we.toString());
		}
	...
	public static DateTime buildDateCell(WritableSheet sheet,
			WritableCellFormat format, Object data, int colonna, int riga)
	...
	public static Number buildNumericCell(WritableSheet sheet,
			WritableCellFormat format, Object number, int colonna, int riga)
			throws WriteException {
		return buildNumericCell(sheet, format, null, number, colonna, riga);
	}

	private static Number buildNumericCell(WritableSheet sheet,
			WritableCellFormat format, WritableCellFormat negativeFormat,
			Object number, int colonna, int riga) throws WriteException {
		Number cell = null;
		if (number != null) {
			Double d=0d;
	...
	public static byte[] generaExcel(ExcelDto excel) 
			throws Exception {
		java.io.File file = null;
		try{
			LOGGER.info("[ExcelUtil::generaExcel] - END");
			
					
			file = new java.io.File(excel.getFileName() + ".xls");
			WritableWorkbook workbook = Workbook.createWorkbook(file);
			WritableSheet ws = workbook.createSheet(excel.getSheetName(), 0);
			...
			DateFormat extenddf = new DateFormat("dd/MM/yyyy hh:mm:ss");
			WritableCellFormat extenddateStyle = new WritableCellFormat(extenddf);
			extenddateStyle.setVerticalAlignment(VerticalAlignment.CENTRE);
				...
				for (ColBodyRow colBodyRow:bodyRow.getListColBodyRow()){
					...
					switch (colBodyRow.getPattern()) {
			            ...
			            case DATE_PATTERN:    
			                ...
			                break;
			            case EXTEND_DATE_PATTERN:    
			            	/*
			            	 * Per ora accetta in input solo XMLGregorianCalendar e Date
			            	 */
			                ExcelUtil.buildDateCell(ws, extenddateStyle, colBodyRow.getValue(), iCol++, row);
			                break;
			            default:
			            	ws.addCell(new Label(iCol++, row, String.valueOf(colBodyRow.getValue()), stringFormat));
			                break;
			...
			LOGGER.error("[ExcelUtil::generaExcel] - exception:"+e.getMessage()+" - "+UtilRebus.getStackTrace(e));
