...
*/
package it.csi.rebus.rebuscrus.web.provider;
...
import it.csi.rebus.rebuscrus.vo.ExceptionVO;
...
public class ExceptionHandler implements ExceptionMapper<RuntimeException> {
	...
	protected Response handleModificaNonAbilitataException(ModificaNonAbilitataException exception) {
		return Response.status(HttpStatus.NOT_ACCEPTABLE.value()).entity(new ExceptionVO(HttpStatus.UNAUTHORIZED.value(), exception)).build();
	}
